...
tiff('PhylogeneticFunctionalAnalysisPicanteR/DiversityRatios_4pan_R1.tif',res=150,width=6,height=6,units='in')
{
  ...
  with(dfdat[dfdat$realm==6 & dfdat$sr_p>0,],points(sr_p,pd_p,pch=17,cex=0.7,col=colx[3]))
  #abline(0,1)
  lmpdsr<-with(dfdat[dfdat$sr_p>0,],lm(pd_p~(sr_p)))
  newdatpd<-data.frame(sr_p=seq(min(dfdat$sr_p[dfdat$sr_p>0]),max(dfdat$sr_p),length.out=100))
  newdatpd$predp1<-predict(lmpdsr,newdata=newdatpd)
  lines(newdatpd$sr_p,newdatpd$predp1)
  mtext(side=3,adj=0,'(a)',line=0.5,cex=1)
  ...
  with(dfdat[dfdat$realm==6 & dfdat$sr_p>0,],points(sr_p,fd_p,pch=17,cex=0.7,col=colx[3]))
  #abline(0,1)
  lmfdsr<-with(dfdat[dfdat$sr_p>0,],lm(fd_p~log(sr_p)))
  newdatfd<-data.frame(sr_p=seq(min(dfdat$sr_p[dfdat$sr_p>0]),max(dfdat$sr_p),length.out=100))
  newdatfd$predf1<-predict(lmfdsr,newdata=newdatfd)
  lines(newdatfd$sr_p,newdatfd$predf1)
  mtext(side=3,adj=0,'(b)',line=0.5,cex=1)
  ...
  #abline(0,1)
  mtext(side=3,adj=0,'(c)',line=0.5,cex=1)
  lmfdpd<-with(dfdat[dfdat$sr_p>0,],lm(fd_p~log(pd_p)))
  ...
  with(dfdat[dfdat$realm==6 & dfdat$sr_p>0,],points(sr,funccon,pch=17,cex=0.7,col=colx[3]))
  mtext(side=3,adj=0,'(d)',line=0.5,cex=1)
  lmfcsr<-with(dfdat[dfdat$sr>0,],lm(funccon~sr))
  newdatfc<-data.frame(sr=seq(min(dfdat$sr[dfdat$sr>0]),max(dfdat$sr),length.out=100))
  newdatfc$predfc1<-predict(lmfcsr,newdata=newdatfc)
  lines(newdatfc$sr,newdatfc$predfc1)
