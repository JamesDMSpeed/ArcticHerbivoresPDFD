...
#realms<-readOGR('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\CMEC regions & realms','Regions')
##Correct misspecified prime meridian of realms data
#crs(realms)<-'+proj=eqc +lat_ts=0 +lat_0=0 +lon_0=12 +x_0=0 +y_0=0 +datum=WGS84 +pm=0 +units=m +no_defs+ellps=WGS84 +towgs84=0,0,0'
#rl<-crop(spTransform(realms,crs(use1)),use1)
#plot(rl)
...
#write.table(pd_es,'S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\pdes.txt')
pd_es<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\pdes.txt')
pd_es[pd_es$ntaxa>0,]
...
#FD
functree<-read.tree('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\Final trait classification\\arcticherbivorefunctiontree.nwk')
plot(functree)
...
#write.table(fd_es,'S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\fdes.txt')
fd_es<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\fdes.txt')
fd_es[fd_es$ntaxa>0,]
...
plot(sr_p*70,fd_p/pd_p,xlab='Species richness',ylab='Functional dispersion',las=1,ylim=c(0,5),type='n')
...
#write.table(fdpdrat,'S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\fdpd_es.txt')
fdpdrat<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\fdpd_es.txt')
...
#write.table(fdpd,'S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\fdpd_es.txt')
fdpd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\fdpd_es.txt')
fdpd[fdpd$ntaxa>1,]
fdpdras<-raster(use1)
...
#writeRaster(divstack2,'S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\DiversityStack')
divstack2<-stack('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\DiversityStack')
divstack2<-mask(divstack2,envvars1$CurrentIce,maskvalue=1)
names(divstack2)<-c('Species richness','Phylogenetic diversity','Functional diversity','Functional/Phylognetic diversity')
my.at <- seq(0, 1, by = 0.1)
levelplot(divstack2,at=my.at,par.settings=YlOrRdTheme())
quantile(divstack2)
  
tiff('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\DiversityMaps.tif',width = 6,height=6,units='in',res=300)
p1 <- levelplot(divstack2[[1]], at=my.at,par.settings=YlOrRdTheme(),scales=list(draw=FALSE), margin=FALSE,main=list('Species richness',cex=0.8))+
  layer(sp.points(np,col=1))+
  layer(sp.polygons(allarc,lwd=0.5,col=grey(0.5)))
p2 <- levelplot(divstack2[[2]], at=my.at, par.settings=YlOrRdTheme(),scales=list(draw=FALSE), margin=FALSE,main=list('Phylogenetic diversity',cex=0.8))+
  layer(sp.points(np,col=1))+
  layer(sp.polygons(allarc,lwd=0.5,col=grey(0.5)))
p3 <- levelplot(divstack2[[3]], at=my.at, par.settings=YlOrRdTheme(),scales=list(draw=FALSE), margin=FALSE,main=list('Functional diversity',cex=0.8))+
  layer(sp.points(np,col=1))+
  layer(sp.polygons(allarc,lwd=0.5,col=grey(0.5)))
p4 <- levelplot(divstack2[[4]],  par.settings=BTCTheme(region=rev(BTC(9))),scales=list(draw=FALSE), margin=FALSE,main=list('Functional dispersion',cex=0.8))+
  layer(sp.points(np,col=1))+
  layer(sp.polygons(allarc,lwd=0.5,col=grey(0.5)))
...
names(effsizestack)<-c('Phylogenetic diversity_es','Functional diversity_es','Functional/Phylogenetic diversity_es')
...
#writeRaster(randomstack,'S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\RandomStack')
...
names(randomstack)<-c('Phylogenetic diversity_rand','Functional diversity_rand','Functional/Phylogenetic diversity_rand')
...
tiff('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\DiversityEffectSizeMaps.tif',width = 8,height=3,units='in',res=300)
...
effsigplot<-levelplot(effsizestack,scales=list(draw=FALSE),names.attr=c('Phylogenetic diversity','Functional diversity','Functional dispersion'),main='', par.strip.text=p.strip)+
  layer(sp.polygons(allarc,lwd=0.5,col=grey(0.5)))+
...
alllayers<-stack(divstack2,effsizestack,randomstack,realms,envvars1)
...
standdf<-cbind(alldatadf[,1:2],scale(alldatadf[,3:28],scale=T,center=T))
...
par(mfrow=c(4,7))
plot(alldatadf$Species.richness~alldatadf$NDVI,ylab='Species richness',xlab='NDVI')
plot(alldatadf$Species.richness~alldatadf$HerbivorePredators_R,ylab='Species richness',xlab='Predator diversity')
...
plot(alldatadf$Phylogenetic.diversity~alldatadf$NDVI,ylab='Phylogenetic diversity',xlab='NDVI')
plot(alldatadf$Phylogenetic.diversity~alldatadf$HerbivorePredators_R,ylab='Phylogenetic diversity',xlab='Predator diversity')
...
plot(alldatadf$Functional.diversity~alldatadf$NDVI,ylab='Functional diversity',xlab='NDVI')
plot(alldatadf$Functional.diversity~alldatadf$HerbivorePredators_R,ylab='Functional diversity',xlab='Predator diversity')
...
plot(alldatadf$Functional.Phylognetic.diversity~alldatadf$NDVI,ylab='Functional dispersion',xlab='NDVI')
plot(alldatadf$Functional.Phylognetic.diversity~alldatadf$HerbivorePredators_R,ylab='Functional dispersion',xlab='Predator diversity')
...
require(MuMIn)
#Full models
fullmod_sr<-with(standdf[!is.na(standdf$TopographicHet),],
                 lm(Species.richness~NDVI+WinterMinTemp+
                      HabitatHet+TopographicHet+
                    IceFreeHistory+HerbivorePredators,
                     na.action=na.fail))
                      
fullmod_pd<-with(standdf[!is.na(standdf$TopographicHet),],
                 lm(Phylogenetic.diversity~NDVI+WinterMinTemp+
                      HabitatHet+TopographicHet+
                      IceFreeHistory+
                      HerbivorePredators,na.action=na.fail))

fullmod_fd<-with(standdf[!is.na(standdf$TopographicHet),],
                 lm(Functional.diversity~NDVI+WinterMinTemp+
                      HabitatHet+TopographicHet+
                      IceFreeHistory+
                      HerbivorePredators,na.action=na.fail))

fullmod_fdpd<-with(standdf[!is.na(standdf$TopographicHet),],
                 lm(Functional.Phylognetic.diversity~NDVI+WinterMinTemp+
                      HabitatHet+TopographicHet+
                      IceFreeHistory+
                      HerbivorePredators,na.action=na.fail))

#Dredge all models between null and full model

modset_pd<-dredge(fullmod_pd)
modset_fd<-dredge(fullmod_fd)
modset_fdpd<-dredge(fullmod_fdpd)

#Model selection
modsel_sr<-model.sel(modset_sr)
modsel_pd<-model.sel(modset_pd)
modsel_fd<-model.sel(modset_fd)
modsel_fdpd<-model.sel(modset_fdpd)

#Model averaging
modavg_sr<-model.avg(modset_sr)
modavg_pd<-model.avg(modset_pd)
modavg_fd<-model.avg(modset_fd)
modavg_fdpd<-model.avg(modset_fdpd)

#Variable importance
imp<-cbind('Species richness'=importance(modavg_sr),
           'Phylogenetic diversity'=importance(modavg_pd),
           'Functional diversity'=importance(modavg_fd),
           'Functional:Phylogenetic diversity'=importance(modavg_fdpd))

m1<-merge(t(as.data.frame(importance(modavg_sr))),t(as.data.frame(importance(modavg_pd))),all=T)
m2<-merge(m1,t(as.data.frame(importance(modavg_fd))),all=T)
m3<-merge(m2,t(as.data.frame(importance(modavg_fdpd))),all=T)
imp<-t(m3)
colnames(imp)<-c('SR','PD','FD','FDPD')
imp<-imp[order(-rowSums(imp)),]

par(mar=c(5,12,3,1))
barplot(t(as.matrix(imp)),col=c(grey(0.8),'pink4','red','darkred'), horiz=T,las=1,main='Relative variable importance'
        ,beside=T,legend.text=T,args.legend=list(y=nrow(imp)-4,x=-0.05,title='Diversity',legend=c('Functional:Phylogenetic','Functional','Phylogenetic','Species')))
...
macplot_est<-rbind(SR=summary(modavg_sr)$coefmat.full[,1],PD=summary(modavg_pd)$coefmat.full[,1],FD=summary(modavg_fd)$coefmat.full[,1],FDPD=summary(modavg_fdpd)$coefmat.full[,1])
macplot_se<-rbind(SR=summary(modavg_sr)$coefmat.full[,2],PD=summary(modavg_pd)$coefmat.full[,2],FD=summary(modavg_fd)$coefmat.full[,2],FDPD=summary(modavg_fdpd)$coefmat.full[,2])
macplot_p<-rbind(SR=summary(modavg_sr)$coefmat.full[,5],PD=summary(modavg_pd)$coefmat.full[,5],FD=summary(modavg_fd)$coefmat.full[,5],FDPD=summary(modavg_fdpd)$coefmat.full[,5])
macplot_p[macplot_p>=0.05]<-1
macplot_p[macplot_p<0.05]<-16
par(mar=c(5,12,1,1))
b1<-barplot(macplot_est[,2:ncol(macplot_est)],horiz=T,col=F,border=F,las=1,xlim=c(-0.6,0.8)
...
arrows(macplot_est[,2:ncol(macplot_est)]+macplot_se[,2:ncol(macplot_est)],b1,
       macplot_est[,2:ncol(macplot_est)]-macplot_se[,2:ncol(macplot_est)],b1,
...
#Effect size models
#Note remove more cells as those with n=1 were not included in ES
fullmod_pdes<-with(standdf[!is.na(standdf$TopographicHet) & !is.na(standdf$Phylogenetic.diversity_es),],
                 lm(Phylogenetic.diversity_es~NDVI+WinterMinTemp+
                      HabitatHet+TopographicHet+
                      IceFreeHistory+
                      HerbivorePredators,na.action=na.fail))

fullmod_fdes<-with(standdf[!is.na(standdf$TopographicHet)& !is.na(standdf$Phylogenetic.diversity_es),],
                 lm(Functional.diversity_es~NDVI+WinterMinTemp+
                      HabitatHet+TopographicHet+
                      IceFreeHistory+
                      HerbivorePredators,na.action=na.fail))

fullmod_fdpdes<-with(standdf[!is.na(standdf$TopographicHet)& !is.na(standdf$Phylogenetic.diversity_es),],
                   lm(Functional.Phylogenetic.diversity_es~NDVI+WinterMinTemp+
                        HabitatHet+TopographicHet+
                        IceFreeHistory+
                        HerbivorePredators,na.action=na.fail))

#Dredge all models between null and full model
modset_pdes<-dredge(fullmod_pdes)
modset_fdes<-dredge(fullmod_fdes)
modset_fdpdes<-dredge(fullmod_fdpdes)

#Model selection
modsel_pdes<-model.sel(modset_pdes)
modsel_fdes<-model.sel(modset_fdes)
modsel_fdpdes<-model.sel(modset_fdpdes)

#Model averaging
modavg_pdes<-model.avg(modset_pdes)
modavg_fdes<-model.avg(modset_fdes)
modavg_fdpdes<-model.avg(modset_fdpdes)

#Variable importance
#impes<-cbind('Phylogenetic effect size'=importance(modavg_pdes),
#           'Functional effect size'=importance(modavg_fdes),
#           'Functional:Phylogenetic effect size'=importance(modavg_fdpdes))

mes1<-merge(t(as.data.frame(importance(modavg_pdes))),t(as.data.frame(importance(modavg_fdes))),all=T)
mes2<-merge(mes1,t(as.data.frame(importance(modavg_fdpdes))),all=T)
impes<-t(mes2)
colnames(impes)<-c('PDes','FDes','FDPDes')
impes<-impes[order(-rowSums(impes)),]

par(mar=c(5,12,3,1))
barplot(t(as.matrix(impes)),col=c('pink4','red','darkred'), horiz=T,las=1,main='Relative variable importance'
        ,beside=T,legend.text=T,args.legend=list(y=nrow(imp)-4,x=-0.05,title='Diversity',legend=c('Functional:Phylogenetic','Functional','Phylogenetic','Species')))

#Model averaged coefficients
summary(modavg_pdes)
summary(modavg_fdes)
summary(modavg_fdpdes)

esmacplot_est<-rbind(PD=summary(modavg_pdes)$coefmat.full[,1],FD=summary(modavg_fdes)$coefmat.full[,1],FDPD=summary(modavg_fdpdes)$coefmat.full[,1])
esmacplot_se<-rbind(PD=summary(modavg_pdes)$coefmat.full[,2],FD=summary(modavg_fdes)$coefmat.full[,2],FDPD=summary(modavg_fdpdes)$coefmat.full[,2])
esmacplot_p<-rbind(PD=summary(modavg_pdes)$coefmat.full[,5],FD=summary(modavg_fdes)$coefmat.full[,5],FDPD=summary(modavg_fdpdes)$coefmat.full[,5])
esmacplot_p[esmacplot_p>=0.05]<-1
esmacplot_p[esmacplot_p<0.05]<-16
par(mar=c(5,12,1,1))
b1<-barplot(esmacplot_est[,2:ncol(esmacplot_est)],horiz=T,col=F,border=F,las=1,xlim=c(-0.8,0.8)
            ,xlab='Model averaged coefficients',beside=T,space=c(1,10))
points(esmacplot_est[,2:ncol(esmacplot_est)],b1,col=c('orange','blue','pink4'),pch=esmacplot_p[,2:ncol(esmacplot_p)]) 
arrows(esmacplot_est[,2:ncol(esmacplot_est)]+esmacplot_se[,2:ncol(esmacplot_est)],b1,
       esmacplot_est[,2:ncol(esmacplot_est)]-esmacplot_se[,2:ncol(esmacplot_est)],b1,
...
par(mar=c(5,12,3,1)){
b1<-barplot(macplot_est[,2:ncol(macplot_est)],horiz=T,col=F,border=F,las=1,xlim=c(-0.6,0.8)
            ,xlab='Model averaged coefficients',beside=T,space=c(1,10),main='Diversity')
points(macplot_est[,2:ncol(macplot_est)],b1,col=c('black','orange','blue','pink4'),pch=macplot_p[,2:ncol(macplot_p)]) 
arrows(macplot_est[,2:ncol(macplot_est)]+macplot_se[,2:ncol(macplot_est)],b1,
       macplot_est[,2:ncol(macplot_est)]-macplot_se[,2:ncol(macplot_est)],b1,
       angle=90,length=0.05,code=3,col=c('black','orange','blue','pink4'))
...
legend('topl',pch=c(1,16),col=1,c('P>=0.05','P<0.05'),cex=0.7,title='Significance')
b1<-barplot(esmacplot_est[,2:ncol(esmacplot_est)],horiz=T,col=F,border=F,las=1,xlim=c(-0.8,0.8)
            ,xlab='Model averaged coefficients',beside=T,space=c(1,10),main='Effect size')
points(esmacplot_est[,2:ncol(esmacplot_est)],b1,col=c('orange','blue','pink4'),pch=esmacplot_p[,2:ncol(esmacplot_p)]) 
arrows(esmacplot_est[,2:ncol(esmacplot_est)]+esmacplot_se[,2:ncol(esmacplot_est)],b1,
       esmacplot_est[,2:ncol(esmacplot_est)]-esmacplot_se[,2:ncol(esmacplot_est)],b1,
       angle=90,length=0.05,code=3,col=c('orange','blue','pink4'))
...
#FDPD
fullgls_fdpd<-gls(Functional.Phylognetic.diversity~NDVI+WinterMinTemp+
                    HabitatHet+TopographicHet+
                    IceFreeHistory+
                    HerbivorePredators_R + as.factor(Regions),correlation=corExp(form=~x+y, nugget=T), method="ML",data=modeldf)

globmodgls_fdpd<-gls(Functional.Phylognetic.diversity~NDVI+WinterMinTemp+
                       HabitatHet+TopographicHet+
                       IceFreeHistory+HerbivorePredators_R + as.factor(Regions),
...
# m1<-merge(t(isr),t(ipd),all=T)
# m2<-merge(m1,t(ifd),all=T)
# m3<-merge(m2,t(ifdpd),all=T)
# 
# impdiv<-t(m3)
# #impdiv<-impdiv[order(-rowSums(impdiv[,1])),]
# impdiv<-impdiv[order(-impdiv[,1]),]
# colnames(impdiv)<-c('FDPD','FD','PD','SR')
# 
# impdiv<-impdiv[,ncol(impdiv):1]
#colsImp<-c(grey(0.8),'orange','blue','pink4')
colsImp<-brewer.pal(5,'Blues')[2:5]
par(mar=c(5,12,1,1))
barplot(t(as.matrix(impdivsort)), horiz=T,las=1,xlab='Relative variable importance',col=colsImp#,col=c('darkred','red','pink4',grey(0.8))
...
tiff('~/DISENTANGLE/WP3/Arctic/AnalysisJan2018/DivImportance.tif',res=300,units='in',width=6,height=6)
par(mar=c(5,12,1,1))
...
#Diversity
isr<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\importance_gls_sr.txt')
ipd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\importance_gls_pd.txt')
ifd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\importance_gls_fd.txt')
ifdpd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\importance_gls_fdpd.txt')
#Give names
colnames(isr)<-'Species'
colnames(ipd)<-'Phylogenetic'
colnames(ifd)<-'Functional'
...
par(mar=c(5,12,1,1))
barplot(t(as.matrix(impdivsort)), horiz=T,las=1,xlab='Relative variable importance',col=colsImp#,col=c('darkred','red','pink4',grey(0.8))
        ,beside=T,legend.text=T,args.legend=list(y=nrow(impdivsort)-5,x=-0.05,title='Diversity',legend=rev(c('Species','Phylogenetic','Functional','Functional:Phylogenetic'))))

#Model averaged coefficients
#Diversity 
#Full coefficents
fullcoefsr<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\modcoef_gls_sr.txt')
fullcoefpd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\modcoef_gls_pd.txt')
fullcoeffd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\modcoef_gls_fd.txt')
fullcoeffdpd<-read.table('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\modcoef_gls_fdpd.txt')
#Reorder rows
...
macplot_est<-macplot_est[,c(1,7,5,8,4,6,9,2,3)]
colnames(macplot_est)<-c('Intercept','NDVI','Predator diversity (R)','Topographic heterogeneity','Habitat heterogeneity','Ice-free history','Winter minimum temperature','Region:Eur vs. Arc','Region: NA vs. Arc')
macplot_se<-rbind(SR=fullcoefsr1[,2],PD=fullcoefpd1[,2],FD=fullcoeffd1[,2],FDPD=fullcoeffdpd1[,2])
macplot_se<-macplot_se[,c(1,7,5,8,4,6,9,2,3)]
macplot_p<-rbind(SR=fullcoefsr1[,5],PD=fullcoefpd1[,5],FD=fullcoeffd1[,5],FDPD=fullcoeffdpd1[,5])
macplot_p<-macplot_p[,c(1,7,5,8,4,6,9,2,3)]
macplot_p[macplot_p>=0.05]<-1
macplot_p[macplot_p<0.05]<-16
...
x11(12,6)
tiff('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\VarImpModAvgCoef.tif',width = 8,height=5,units='in',res=150,pointsize=8)
par(mfrow=c(1,2))
par(mar=c(5,12,1,1))
#Imp
bI<-barplot(cbind(t(as.matrix(impdivsort)),rep(NA,times=4)), horiz=T,las=1,xlab='Relative variable importance',col=colsImp#,col=c('darkred','red','pink4',grey(0.8))
        ,beside=T,legend.text=T,args.legend=list(cex=0.9,'topr',title='Diversity',legend=rev(c('Species','Phylogenetic','Functional','Functional dispersion'))))
par(mar=c(5,2,1,1))
par(xpd=T)
bI1<-barplot(cbind(t(as.matrix(impdivsort)),rep(NA,times=4)), horiz=T,beside=T,xlim=c(-0.5,0.5),col=F,border=F,
             xlab='Model averaged coefficients',las=1,names.arg=NA)#,names.arg=colnames(macplot_est[,2:ncol(macplot_est)]))
...
cluster8<-stack('S:\\DISENTANGLE\\WP3\\ArcticHerbivoreFDPD\\PhylogeneticFunctionalAnalysisPicanteR\\cluster8rasters')
